...
export async function GET(request, { params }) {
    ...
    try {
        const [results, fields] = await dbPool.query(`
            SELECT m.id, m.modelo, motos.marcas.nome AS marca, m.ano, m.categoria, m.codigo_fipe, m.preco_fipe, m.tipo_motor, m.cilindrada, m.potencia_A, m.potencia_G, m.torque_A, 
            ...
            JOIN motos.marcas ON m.marca = motos.marcas.id
            WHERE m.modelo = ? 
              AND m.ano = ?;
            `,
            [modelo, ano]);
